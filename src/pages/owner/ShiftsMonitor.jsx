  ...
  const fetchTabData = async () => {
    ...
    try {
      const [shift, history] = await Promise.all([
        ...
        ownerReportService
          .getShiftsHistory(selectedBranch)
          .catch(() => ({ shifts: [] })),
      ]);
          ...
          {isDataLoading ? (
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-12 text-center shadow-sm max-w-2xl mx-auto transition-colors">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">
                لم يقم أي كاشير بفتح وردية في هذا الفرع حتى الآن.
              </p>
            </div>
          ) : (
              ...
              <div className="flex items-center gap-4 w-full xl:w-auto">
                ...
                <div>
                  ...
                  {(() => {
                    ...
                    const leaverName =
                      previousShift?.acknowledged_by?.name?.split(' ')[0] ||
                      liveShift.cashier_id?.name?.split(' ')[0] ||
                      'السابق';
                    const takerName =
                      liveShift.acknowledged_by?.name?.split(' ')[0] ||
                      'الحالي';
                        ...
                        {!liveShift.acknowledged_at ? (
                          <div className="flex items-center gap-1.5 text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 px-2.5 py-2 rounded-lg border border-amber-200 dark:border-amber-500/20 shadow-sm">
                            ...
                            <span>
                              ...
                              في انتظار استلام الزميل الآخر للدرج منذ:{' '}
                              <span className="font-black tracking-wider">
                                {new Date(
                                  liveShift.start_time
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 w-full xl:w-auto border-t xl:border-t-0 border-slate-100 dark:border-slate-700 pt-4 xl:pt-0">
                <div className="flex gap-6 w-full sm:w-auto justify-between sm:justify-start px-2 sm:px-0">
                  <div className="text-right sm:border-l border-slate-100 dark:border-slate-700 sm:pl-6">
                    ...
                    <p className="text-lg font-black text-emerald-600 dark:text-emerald-400">
                      {liveShift.starting_cash.toLocaleString()} ج
                    </p>
                  </div>
                ...
                <div className="flex gap-2 w-full sm:w-auto mt-2 sm:mt-0">
                  <button
                    ...
                    className="p-3 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-600 active:scale-95 transition-all"
                    title="تحديث الأرقام"
                  >
                    <RefreshCw className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => handleViewDetails(liveShift, true)}
                    className="flex-1 sm:flex-none py-3 px-5 bg-slate-800 hover:bg-slate-900 dark:bg-indigo-600 dark:hover:bg-indigo-700 text-white font-semibold rounded-xl transition-all flex items-center justify-center gap-2 active:scale-95 text-sm shadow-sm"
                  >
          ...
          {isDataLoading ? (
            ...
            <div className="text-center py-20 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm max-w-2xl mx-auto transition-colors">
              ...
              <p className="font-semibold text-slate-500 dark:text-slate-400">
                لا توجد ورديات مغلقة مسجلة في هذا الفرع.
              </p>
            </div>
          ) : (
            <div className="flex flex-col gap-4">
              ...
              {Object.entries(groupedShifts).map(
                ([monthYear, shiftsInMonth]) => {
                  const isExpanded = expandedMonth === monthYear;

                  return (
                    <div
                      key={monthYear}
                      className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-[24px] overflow-hidden shadow-sm transition-colors"
                    >
                      {/* 📅 هيدر الشهر القابل للضغط */}
                      <button
                        onClick={() =>
                          setExpandedMonth(isExpanded ? null : monthYear)
                        }
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div
                            className={`p-3 rounded-xl transition-colors ${isExpanded ? 'bg-indigo-600 dark:bg-indigo-500 text-white shadow-md shadow-indigo-500/20 dark:shadow-none' : 'bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400'}`}
                          >
                            <CalendarDays className="w-5 h-5" />
                          </div>
                          <div className="text-right">
                            <h3
                              className={`text-lg font-black transition-colors ${isExpanded ? 'text-indigo-900 dark:text-indigo-300' : 'text-slate-800 dark:text-white'}`}
                            >
                              سجلات {monthYear}
                            </h3>
                        ...
                        <div
                          className={`p-2 rounded-full transition-transform duration-300 ${isExpanded ? 'rotate-180 bg-indigo-100 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400' : 'bg-slate-50 dark:bg-slate-900 text-slate-400 dark:text-slate-500'}`}
                        >
                          ...
                          >
                            <div className="p-5 grid grid-cols-1 lg:grid-cols-2 gap-4">
                              {shiftsInMonth.map((shift) => {
                                const currentName =
                                  shift.acknowledged_by?.name?.split(' ')[0] ||
                                  'العامل';
                                const nextShift = getNextShiftInfo(shift._id);
                                const nextName =
                                  nextShift?.acknowledged_by?.name?.split(
                                    ' '
                                  )[0] || 'الزميل القادم';
                                ...
                                return (
                                  <div
                                    key={shift._id}
                                    className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow flex flex-col relative"
                                  >
                                    ...
                                    <div className="flex flex-col gap-1.5 bg-slate-50 dark:bg-slate-900/50 p-3 rounded-xl border border-slate-100 dark:border-slate-700/50 my-4">
                                      <div
                                        className={`flex justify-between items-center text-[10px] font-black ${shift.acknowledged_at ? 'text-emerald-700 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'}`}
                                      >
                                        <span className="flex items-center gap-1.5">
                                          <ShieldCheck className="w-3.5 h-3.5" />{' '}
                                          1- استلام ({currentName}):
                                        </span>
                                        <span>
                                      ...
                                      <div
                                        className={`flex justify-between items-center text-[10px] font-black ${nextShift?.acknowledged_at ? 'text-indigo-600 dark:text-indigo-400' : 'text-rose-500 dark:text-rose-400'}`}
                                      >
                                        <span className="flex items-center gap-1.5">
            ...
            >
              <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 shrink-0">
                <div className="flex justify-between items-start">
                  ...
                  <button
                    onClick={() => setSelectedArchiveShift(null)}
                    className="p-2 bg-white dark:bg-slate-700 text-slate-400 dark:text-slate-300 rounded-lg hover:text-rose-500 dark:hover:text-rose-400 shadow-sm border border-slate-100 dark:border-slate-600 transition-all active:scale-95"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
                ...
                {(() => {
                  const currentName =
                    selectedArchiveShift.acknowledged_by?.name?.split(' ')[0] ||
                    'العامل';
                  const nextShift = getNextShiftInfo(selectedArchiveShift._id);
                  const nextName =
                    nextShift?.acknowledged_by?.name?.split(' ')[0] ||
                    'الزميل القادم';
                  ...
                  return (
                    <div className="flex flex-wrap items-center gap-2 mt-4 text-[10px] sm:text-[11px] font-bold bg-white dark:bg-slate-800 p-3 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                      ...
                      <span className="bg-slate-50 dark:bg-slate-700/50 text-slate-600 dark:text-slate-300 px-2.5 py-2 rounded-lg border border-slate-200 dark:border-slate-700 flex items-center gap-1.5">
                        ...
                        2- تسليم {currentName} للدرج:{' '}
                        <span className="font-black text-slate-800 dark:text-white">
                          {selectedArchiveShift.end_time
                            ? new Date(
                                selectedArchiveShift.end_time
                              ).toLocaleTimeString('ar-EG', {
                                hour: '2-digit',
                      ...
                      <span
                        className={`px-2.5 py-2 rounded-lg border flex items-center gap-1.5 ${nextShift?.acknowledged_at ? 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 border-indigo-200 dark:border-indigo-500/20' : 'bg-rose-50 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400 border-rose-200 dark:border-rose-500/20'}`}
                      >
                        <CheckCircle2 className="w-4 h-4" />
                        3- حضور واستلام {nextName}:{' '}
                        <span className="font-black">
                          {nextShift?.acknowledged_at
                            ? new Date(
                                nextShift.acknowledged_at
                      ...
                      .filter(
                        (item) =>
                          !(
                            ...
                            item.description?.includes('دفعة مستقطعة من فاتورة')
                          )
                      )
                      .map((item, idx) => (
                          ...
                          <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 p-3.5 rounded-xl shadow-sm flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 hover:border-slate-200 dark:hover:border-slate-600 transition-colors">
                            <div className="flex-1">
                              ...
                              <h5 className="text-sm font-bold text-slate-800 dark:text-white">
                                {item.domain === 'CASH' &&
                                item.type === 'VENDOR_PAYMENT'
                                  ? `سداد للمورد: ${item.person_name}`
                                  : item.domain === 'CASH' &&
                                      item.type === 'INCOME'
                                    ? `إيراد/سداد دين من: ${item.person_name}`
                                    : item.domain === 'CASH' &&
                                        item.category === 'PERSONAL'
                                      ? `سلفة للعامل: ${item.person_name}`
                                      : item.domain === 'CASH'
                                        ? `مصروف تشغيل`
                                        : item.domain === 'INVOICE'
                                          ? `استلام بضاعة آجلة من ${item.company || item.person_name}`
                                          : item.domain === 'CUSTOMER_DEBT' &&
                                              item.type === 'CREDIT'
                                            ? `سحب شكك: ${item.person_name}`
                              ...
                              {item.domain === 'INVOICE' && (
                                <div className="mt-2 inline-block bg-slate-50 dark:bg-slate-700/50 px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-600">
                                  <p className="text-[10px] font-bold text-slate-600 dark:text-slate-300">
                                    إجمالي: {item.amount.toLocaleString()} |{' '}
                              ...
                              {item.image && (
                                <a
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="inline-flex items-center gap-1 text-[11px] text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 font-medium mt-1.5 bg-blue-50 dark:bg-blue-500/10 px-2 py-0.5 rounded"
                                >
              ...
              <div className="p-4 border-t border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 shrink-0">
                <button
                  onClick={() => setSelectedArchiveShift(null)}
                  className="w-full py-3.5 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 font-bold rounded-xl hover:bg-slate-100 dark:hover:bg-slate-600 hover:text-indigo-600 dark:hover:text-indigo-400 active:scale-95 transition-all text-sm flex items-center justify-center gap-2 shadow-sm"
                >
